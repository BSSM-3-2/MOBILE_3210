...
import { Post, Comment, Reply, ImageType } from '@type/Post';
...
function normalizeImage(image: unknown): ImageType | null {
    if (!image) return null;

    if (typeof image === 'string') {
        return { type: 'REMOTE', url: image };
    }
    ...
    if (typeof image === 'object') {
        ...
        if (candidate.type === 'REMOTE' && typeof candidate.url === 'string') {
            return { type: 'REMOTE', url: candidate.url };
        }

        if (candidate.type === 'LOCAL' && candidate.source) {
            return { type: 'LOCAL', source: candidate.source as never };
        }
